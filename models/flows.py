...
class NormalizingFlowModel2(nn.Module):
    """ A Normalizing Flow Model is a (prior, flow) pair """
    
    def __init__(self, mu, log_sigma, flows):
        super().__init__()
    ...
    def get_covar(self, i):
        sg = torch.exp(self.log_sigma)[i, :]
        sg = (sg @ sg.T) + torch.eye(len(sg))*1e-6
        return sg[None, ...]
    
    def forward(self, x):
        self.base_dist = D.MultivariateNormal(self.mu,
             torch.cat([self.get_covar(i) for i in range(len(self.log_sigma))], axis=0))
        #self.base_dist = D.MultivariateNormal(self.mu, torch.exp(self.log_sigma))
        zs, log_det = self.flow.forward(x)
        base_logprob = self.base_dist.log_prob(zs[-1]).view(x.size(0), -1).sum(1)
