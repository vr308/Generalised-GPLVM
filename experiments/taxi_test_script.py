...
# -*- coding: utf-8 -*-
"""
Script for experiments with taxi data
...
class TaxiModel(BayesianGPLVM):
     def __init__(self, n, data_dim, latent_dim, n_inducing, X, nn_layers=None):
        ...
        super(TaxiModel, self).__init__(X, q_f)
...
for _ in range(1):
    ...
    data = pd.read_csv('data/taxi_count_data.csv')
    Y = np.array(data[['yellow', 'green', 'fhv']], dtype=int)

    N = len(Y); d = len(Y.T); m = 36; 
    ...
    for model_name in models:
        ...
        if pca == True:
              ...
              X_init = torch.nn.Parameter(torch.ones(N, latent_dim)*0.1)
        ...
        if model_name == 'point':
            ...
            X = NNEncoder(n_train, latent_dim, prior_x, data_dim, layers=nn_layers)
            
        # Initialise model, likelihood, elbo and optimizer
        
        model = TaxiModel(N, data_dim, latent_dim, n_inducing, X, nn_layers=nn_layers)
        ...
        for i in iterator:
            ...
            for loss_iter in range(10):
                ...
                loss = loss - elbo(output_batch, Y_train[batch_index].T).sum()
        ...
        filename = f'taxi_{model_name}_{SEED}.pkl'
        ...
        #Compute latent test & reconstructions
            ...
            if ae is True:
                if model_name != 'iaf':
                    model_jitter = model.X.jitter.clone()
                    model.X.jitter = model.X.jitter[:n_test]
                ...
                losses_test,  X_test = model.predict_latent(Y_train, Y_test, optimizer.defaults['lr'], 
                                        ...
                                        model_name=model_name,pca=pca, steps=10000)
            ...
            mse_train = rmse(Y_train, Y_train_recon.T.exp())
            mse_test = rmse(Y_test, Y_test_recon.T.exp())
